...
class ProfileCompensationRepository {
  ...
  ProfileTeamMember _teamMemberFromMap(
    ...
    final roles = (map['roles'] as List?) ?? const <dynamic>[];
    final roleFromRoles = roles
        .whereType<Map>()
        .map((role) => _cleanText(Map<String, dynamic>.from(role)['label']))
        .where((label) => label.isNotEmpty)
        .join(', ');
    final roleText = _cleanText(map['role']).isNotEmpty
        ? _cleanText(map['role'])
        : roleFromRoles.isNotEmpty
            ? roleFromRoles
