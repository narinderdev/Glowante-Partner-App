...
class _TeamScreenState extends State<TeamScreen> {
  ...
  List<Map<String, dynamic>> _teamAppointmentMaps(dynamic raw) {
    if (raw is List) {
      return raw
          .whereType<Map>()
    ...
    if (raw is Map<String, dynamic>) {
      ...
      if (nestedData is Map<String, dynamic>) {
        final appointments = nestedData['appointments'];
      ...
      final appointments = raw['appointments'];
      if (appointments is List) {
        return appointments
            .whereType<Map>()
            .map((item) => Map<String, dynamic>.from(item))
    ...
    if (raw is Map) {
      return _teamAppointmentMaps(Map<String, dynamic>.from(raw));
    }
    return const <Map<String, dynamic>>[];
  }

  List<Map<String, dynamic>> _teamAppointmentItems(
    Map<String, dynamic> appointment,
  ) {
    final rawItems = appointment['items'];
  ...
  void _collectBookedMemberIdsFromSource(
    ...
    addId(source['assignedUserId']);
    addId(source['teamMemberId']);
    addId(source['professionalUserId']);
    addId(source['userId']);
