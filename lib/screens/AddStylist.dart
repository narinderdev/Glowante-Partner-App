...
Future<void> _pickImage() async {
  ...
  if (pickedFile != null) {
    ...
    imageUrl = await _uploadImageToS3(_cameraImage!);  // Store the image URL after uploading
...
Future<void> _fetchRolesAndSpecializations() async {
  try {
    ...
    Map<String, dynamic> data = await ApiService().getRolesAndSpecializations();
          ...
          children: [
            ...
            Flexible(
              child: ListView.separated(
                shrinkWrap: true,
                itemCount: source.length, // This should match the length of your roles or specialities
                separatorBuilder: (_, __) => const Divider(height: 1),
                itemBuilder: (_, i) {
                  final item = source[i];
                  final itemName = item['label'] ?? item['name'] ?? ''; // Extract label or name
                  final itemId = item['id']; // Extract id
                  final itemCode = item['code']; // Extract code
                  final checked = temp.contains(itemName); // Check if the item is selected

                  // Log the data for each item (id, code, label)
                  print('Item ${itemName}: id = $itemId, code = $itemCode, selected = $checked');
                  ...
                  return CheckboxListTile(
                    ...
                    onChanged: (v) {
                      ...
                      if (v == true && !temp.contains(itemName)) {
                        ...
                        print('Selected ${itemName}: id = $itemId, code = $itemCode');
                      } else if (v == false) {
                        temp.remove(itemName);
                        // Log the deselected item
                        print('Deselected ${itemName}: id = $itemId, code = $itemCode');
                      }
            ...
            _GradientButton(
              ...
              onPressed: () {
                setState(() {
                  target
                    ..clear()
                    ..addAll(temp); // Update the target list with selected items
                });
  ...
  child: GestureDetector(
    ...
    child: CircleAvatar(
      ...
      child: _cameraImage == null
          ? const Icon(Icons.camera_alt, size: 30) // Default camera icon
          : ClipRRect(
                    ...
                    children: [
                      Expanded(
                        child: TextFormField(
                          ...
                          decoration: _decor(hint: 'Verify phone number', prefix: const Icon(Icons.search)),
 ...
 SizedBox(
  ...
  child: _GradientButton(
    ...
    height: 48,
                  ...
                  // OTP Field
                  const Text('Otp', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600)),
                  const SizedBox(height: 8),
                  TextFormField(
                    enabled: false,
                    controller: _otpCtrl,
                  ...
                  const Text('Brief About Member', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600)),
                  const SizedBox(height: 8),
                  TextFormField(
                    controller: _briefCtrl,
                    maxLines: 4,
                    decoration: _decor(hint: 'Enter a brief about this member')
...
print('Form Data: $formData'); // Log the form data
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(
          builder: (context) => ChooseTimeSlot(formData: formData), // Pass data directly to the new screen
        ),
      );
    }
  },
    ...
    return GestureDetector(
      ...
      child: AbsorbPointer(
        child: TextFormField(
          ...
          validator: (_) => values.isEmpty ? 'Please select at least one' : null,
        ),
      ),
    );
  }
